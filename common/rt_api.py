...
class PersistentAssetCache:
    def __init__(self, max_size=1500, ttl=259200):
        """Cache up to 1500 assets for 72 hours (259200 seconds)"""
        self.cache = {}
        ...
        self.save_thread = threading.Thread(
            target=self._periodic_save_loop, daemon=True
        )
    ...
    def set(self, key, value):
        ...
        with self.lock:
            ...
            self.cache[key] = {"data": value, "expires": time.time() + self.ttl}
    ...
    def _cleanup_expired(self):
        ...
        with self.lock:
            expired_keys = [
                key for key, entry in self.cache.items() if entry["expires"] < now
            ]
        ...
        try:
            with self.lock:
                ...
                cache_data = {
                    ...
                    "entries": {
                        k: {"data": v["data"], "expires": v["expires"]}
        ...
        try:
            if self.cache_file.exists():
                with open(self.cache_file, "r") as f:
                    ...
                    expired_count = len(cache_data.get("entries", {})) - len(
                        valid_entries
                    )
                    logger.info(
                        f"Loaded {len(valid_entries)} valid entries from disk cache"
                    )
                    if expired_count > 0:
...
def create_retry_session(
    retries=5, backoff_factor=1.0, status_forcelist=(500, 502, 503, 504)
):
...
def rt_api_request(method, endpoint, data=None, config=None):
    ...
    """
    # Config parameter is optional - uses Django/Flask settings via get_config()
    # Note: config parameter kept for backwards compatibility but get_config() is used

    base_url = get_config("RT_URL")
    api_endpoint = get_config("API_ENDPOINT")
    token = get_config("RT_TOKEN")
        ...
        if resp is not None:
            ...
            if headers:
                try:
                    ...
                    logger.error(
                        "Error response headers present but could not convert to dict"
                    )
        raise
...
def fetch_asset_data(asset_id, config=None, use_cache=False):
    ...
    try:
        ...
        logger.info(f"Fetching asset data for ID: {asset_id} from RT API")
        ...
        if "Name" not in response:
            ...
            logger.warning(
                f"Response for asset ID {asset_id} is missing Name field: {response}"
            )
...
def search_assets(query, config=None, try_post_fallback=True, use_cache=False):
    ...
    try:
        ...
        if isinstance(query, list):
            ...
            logger.info(
                f"Using provided search conditions: {json.dumps(search_condition)}"
            )
        elif query == "id>0":
            search_condition = [{"field": "id", "operator": ">=", "value": 0}]
            logger.info("Using id>0 search condition to fetch all assets")
        elif "=" in query:
            field, value = query.split("=", 1)
            field = field.strip()
            value = value.strip("' ")
            search_condition = [{"field": field, "operator": "=", "value": value}]
            logger.info(f"Using equals search condition: {field}={value}")
        elif "LIKE" in query.upper():
            field, value = query.upper().split("LIKE", 1)
            field = field.strip()
            value = value.strip("' ")
            search_condition = [{"field": field, "operator": "LIKE", "value": value}]
            logger.info(f"Using LIKE search condition: {field} LIKE {value}")
        else:
            search_condition = [{"field": "Name", "operator": "LIKE", "value": query}]
        ...
        while True:
            ...
            try:
                response = rt_api_request(
                    "POST", f"/assets?page={page}", data=search_condition, config=config
                )
                fetch_duration = time.time() - fetch_start
                logger.info(
                    f"Page {page} fetch completed in {fetch_duration:.1f} seconds"
                )
            except Exception as e:
                ...
                if asset_id:
                    try:
                        ...
                        if len(item.keys()) <= 3:  # Only has id, type, _url
                            ...
                            asset_detail = rt_api_request(
                                "GET", f"/asset/{asset_id}", config=config
                            )
                        ...
                        if processed % 10 == 0:
                            logger.info(
                                f"Processed {processed}/{len(items)} items from page {page}"
                            )
                    except Exception as e:
                        logger.warning(
                            f"Failed to fetch details for asset {asset_id}: {e}"
                        )
            ...
            if page % 10 == 0:  # Log progress every 10 pages
                ...
                logger.info(
                    f"Processed {len(all_assets)} assets so far in {duration:.1f} seconds..."
                )

        total_duration = time.time() - start_time
        logger.info(
            f"Found {len(all_assets)} total assets in {total_duration:.1f} seconds"
        )
...
def update_asset_custom_field(asset_id, field_name, field_value, config=None):
    ...
    """
    data = {"CustomFields": [{"name": field_name, "values": [field_value]}]}
...
def update_user_custom_field(user_id, field_name, field_value, config=None):
    ...
    data = {"CustomFields": {field_name: field_value}}
...
def get_assets_by_owner(owner, exclude_id=None, config=None):
    ...
    try:
        ...
        headers = {
            ...
            "Authorization": f"token {get_config('RT_TOKEN')}",
        }
...
def create_ticket(
    subject, body, queue=None, requestor=None, status=None, owner=None, config=None
):
    """
    ...
    try:
        ...
        if queue is None:
            queue = get_config("RT_DEFAULT_QUEUE", "General")

        # Prepare ticket data
        ticket_data = {"Subject": subject, "Queue": queue, "Content": body}
