    ...
    def __init__(self, max_size=1500, ttl=259200):  # Cache up to 1500 assets for 72 hours (259200 seconds)
        ...
        try:
            ...
            try:
                ...
                logger.info(f"Using Flask instance directory for cache: {cache_dir}")
            except (ImportError, RuntimeError):
                # Django approach
                from django.conf import settings
                instance_path = settings.BASE_DIR
                cache_dir = Path(instance_path) / 'cache'
                logger.info(f"Using Django BASE_DIR for cache: {cache_dir}")
        except Exception:
