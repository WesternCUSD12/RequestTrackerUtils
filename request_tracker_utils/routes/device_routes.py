...
@bp.route('/ticket-test/<asset_id>')
def ticket_test(asset_id):
    """Debug endpoint to test ticket creation for a device"""
    try:
        ...
        # Create a test ticket description
        test_description = f"Test ticket created for {asset_name} at {time.strftime('%Y-%m-%d %H:%M:%S')}"
        
        try:
            logger.info(f"Debug: Creating test ticket for asset {asset_id}")
            
            # Create a ticket in the Device Repair queue
            ticket_data = {
                "Queue": "Device Repair",
                "Subject": f"Device Check-in TEST: {asset_name}",
                "Content": test_description,
                "Refers-To": f"asset:{asset_id}"
            }
            
            # Log the ticket creation attempt with detailed data
            logger.info(f"Debug: Creating ticket with data: {json.dumps(ticket_data)}")
            
            # Use the specialized create_ticket function instead of rt_api_request directly
            from ..utils.rt_api import create_ticket
            ticket_response = create_ticket(ticket_data)
            logger.info(f"Debug: Ticket creation response: {json.dumps(ticket_response)}")
            
            # Extract ticket ID from response
            ticket_id = None
            if "id" in ticket_response:
                ticket_id = ticket_response.get("id")
                logger.info(f"Debug: Created ticket ID: {ticket_id}")
                return jsonify({
                    "success": True,
                    "message": f"Test ticket created successfully for asset {asset_name}",
                    "asset": asset_data,
                ...
                return jsonify({
                    "error": "Failed to create ticket - no ticket ID returned",
                    "asset": asset_data,
                    "ticket_data": ticket_data,
                    "ticket_response": ticket_response
                }), 500
            ...
            logger.error(f"Debug: Error creating test ticket: {e}")
            logger.error(traceback.format_exc())
            return jsonify({
                "error": f"Failed to create test ticket: {str(e)}",
                "asset": asset_data,
                "ticket_data": ticket_data if 'ticket_data' in locals() else None,
            }), 500
            
    except Exception as e:
        logger.error(f"Debug: Error in ticket test: {e}")
        logger.error(traceback.format_exc())
        return jsonify({
            "error": f"Failed to process ticket test: {str(e)}"
        }), 500