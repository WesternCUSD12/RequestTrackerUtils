...
def login_view(request: HttpRequest) -> HttpResponse:
    ...
    try:
        ...
        if not success or not user_info:
            ...
            logger.warning(f"Authentication failed for user: {username} from IP: {request.META.get('REMOTE_ADDR')}")
            return render(request, 'auth/login.html', {
        ...
        if not user_role:
            ...
            logger.info(f"access_denied: {username} from IP: {request.META.get('REMOTE_ADDR')}")
            
            return render(request, 'auth/login.html', {
                'error_type': 'Access Denied',
                'error_message': 'You are not authorized to access this application. '
                                'Only members of tech-team or TEACHERS groups may sign in.',
                'username': username,
                'next': next_url
            })
        ...
        logger.warning(f"login_failure: {username} - invalid credentials from IP: {request.META.get('REMOTE_ADDR')}")
        ...
        logger.warning(f"login_failure: {username} - account disabled/locked from IP: {request.META.get('REMOTE_ADDR')}")
        ...
        logger.error(f"login_failure: {username} - LDAP service unavailable from IP: {request.META.get('REMOTE_ADDR')}")
        ...
        # Unexpected error
        logger.error(f"login_failure: {username} - unexpected error: {e} from IP: {request.META.get('REMOTE_ADDR')}", exc_info=True)
