...
class LDAPClient:
    ...
    def __init__(self):
        ...
        self.teacher_group = settings.LDAP_TEACHER_GROUP
    ...
    def _connect(self, username: str, password: str) -> Connection:
        ...
        try:
            ...
            domain_parts = [part.split('=')[1] for part in self.base_dn.split(',') if part.strip().startswith('DC=')]
            ...
            for user_dn in bind_formats:
                try:
                    ...
                    logger.info(f"LDAP connection established for user: {username} using format: {user_dn}")
                    return conn
            ...
            if any(keyword in error_message for keyword in ['disabled', 'locked', 'expired', 'must change password']):
                logger.warning(f"Account issue for user {username}: {error_message}")
                raise LDAPAccountDisabledError("Account is disabled, locked, or requires password change") from e
            ...
            raise LDAPServiceUnavailableError("Authentication service unavailable") from e
    ...
    def authenticate(self, username: str, password: str) -> Tuple[bool, Optional[dict]]:
        ...
        try:
            ...
            logger.info(f"Authentication successful for user: {username} (groups: {groups})")
