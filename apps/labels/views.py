...
from common.rt_api import fetch_asset_data, search_assets, find_asset_by_name, rt_api_request
from common.label_config import LABEL_TEMPLATES
from common.text_utils import truncate_text_to_width
from .utils import generate_qr_code, generate_barcode, get_custom_field_value, get_default_label_size
...
def print_label(request):
    ...
    direct_lookup = request.GET.get('direct', 'false').lower() == 'true'
    size = request.GET.get('size', 'large')
    
    # Validate size parameter
    if size not in ['large', 'small']:
        logger.warning(f"Invalid size parameter: {size}")
        return JsonResponse({"error": "Invalid size parameter. Must be 'large' or 'small'"}, status=400)
    
    # Get template configuration
    template_config = LABEL_TEMPLATES.get(size, LABEL_TEMPLATES['large'])
    logger.info(f"Using {size} label template with config: {template_config.name}")
    
    # Log what we received with higher log level for debugging
    logger.info(f"Received parameters: assetId={asset_id}, assetName={asset_name}, direct={direct_lookup}")
    
    # Check if we have at least one of the required parameters
    if not asset_id and not asset_name:
        logger.info("Missing both assetId and assetName parameters in the request")
        return JsonResponse({"error": "You must provide either assetId or assetName parameter"}, status=400)
    
    # If assetId looks like an asset tag (non-numeric), treat it as an asset name
    if asset_id and not asset_id.isdigit():
        logger.info(f"assetId '{asset_id}' is not numeric - treating as asset name")
        asset_name = asset_id
        asset_id = None
    ...
    try:
        ...
        if asset_name and not asset_id:
            ...
            try:
                logger.info(f"Looking up asset by name using JSON filter: {asset_name}")
                
                # Construct the same filter format used in the curl command
                import requests
                import json
                
                base_url = getattr(settings, 'RT_URL')
                api_endpoint = getattr(settings, 'API_ENDPOINT')
                token = getattr(settings, 'RT_TOKEN')
                ...
                if items and len(items) > 0:
                    ...
                    logger.info(f"Found asset ID: {asset_id} for exact name match: {asset_name}")
                else:
                    ...
                    if '-' in asset_name:
                        ...
                        logger.info(f"Making POST request with prefix filter: {json.dumps(filter_data)}")
                        response = requests.post(url, headers=headers, json=filter_data)
                        response.raise_for_status()
                        # Process the response
                        result = response.json()
                        ...
                        if items and len(items) > 0:
                            ...
                            logger.info(f"Found asset ID: {asset_id} for prefix match: {asset_name_found}")
                        else:
                            # All JSON filter approaches failed
                            if direct_lookup:
                                # Fall back to direct lookup if requested
                                logger.info("JSON filter failed, falling back to direct lookup approach")
                                direct_query = "id>0 LIMIT 1000" 
                                response = rt_api_request("GET", f"/assets?query={urllib.parse.quote(direct_query)}", settings)
                                all_assets = response.get("assets", [])
                                logger.info(f"Retrieved {len(all_assets)} assets for filtering")
                                ...
                                if matching_assets:
                                    ...
                                    asset_id = asset.get('id')
                                    logger.info(f"Direct lookup found asset ID: {asset_id} for name: {asset_name}")
                                else:
                                    ...
                                    if approx_matches:
                                        ...
                                        asset_id = asset.get('id')
                                        logger.info(f"Direct lookup found approximate match with ID: {asset_id}, name: {asset.get('Name')}")
                                    else:
                                        raise Exception(f"No asset found with name similar to: {asset_name}")
                    else:
                        # Fall back to standard find_asset_by_name as last resort
                        logger.info("JSON filter failed, falling back to standard find_asset_by_name")
                        asset = find_asset_by_name(asset_name, settings)
                        if not asset:
                            raise Exception(f"No asset found with name: {asset_name}")
                        asset_id = asset.get('id')
                        logger.info(f"Found asset ID: {asset_id} for name: {asset_name}")
                
            except Exception as e:
                logger.error(f"Error looking up asset by name: {e}")
                return JsonResponse({
        ...
        logger.info(f"Fetching asset data for ID: {asset_id}")
        asset_data = fetch_asset_data(asset_id, settings)
        ...
        asset_label_data = {
            ...
            "label_width": getattr(settings, "LABEL_WIDTH_MM", 100) - 4,
            "label_height": getattr(settings, "LABEL_HEIGHT_MM", 62) - 4
        }
        ...
        try:
            ...
            logger.debug(f"Generating QR code for URL: {rt_asset_url}")
            ...
            logger.debug(f"QR code generation successful (box_size={qr_box_size})")
        ...
        try:
            logger.debug(f"Generating barcode for content: {asset_label_data['name']}")
            ...
            logger.debug(f"Barcode generation successful ({template_config.barcode_width_mm}mm x {template_config.barcode_height_mm}mm)")
        ...
        logger.debug(f"Asset label data: {json_for_logging.dumps(log_data, indent=4)}")
    ...
    template_name = 'labels/small_label.html' if size == 'small' else 'labels/label.html'
    logger.debug(f"Rendering {template_name} template with asset label data")
    ...
    return render(request, template_name, asset_label_data)
...
def batch_labels(request):
    ...
    logger.info(f"Batch labels route called with method: {request.method}")
    logger.info(f"Request POST data: {request.POST}")
    logger.info(f"Request GET data: {request.GET}")
    
    if request.method == 'GET':
        return render(request, 'labels/batch_labels_form.html')
    
    # POST method - process the submitted query or asset names
    query = request.POST.get('query', '')
    asset_names = request.POST.get('asset_names', '')
    direct_lookup = request.POST.get('direct', 'false').lower() == 'true'
    
    logger.info(f"Batch labels request: query={query}, asset_names={asset_names}, direct={direct_lookup}")
    ...
    try:
        ...
        if asset_names:
            ...
            logger.info(f"Processing {len(names_list)} asset names: {', '.join(names_list)}")
            ...
            all_assets = []
            if direct_lookup:
                logger.info("Using direct lookup for batch processing")
                try:
                    # Fetch up to 1000 assets (adjust limit as needed)
                    direct_query = "id>0 LIMIT 1000"
                    response = rt_api_request("GET", f"/assets?query={urllib.parse.quote(direct_query)}", settings)
                    all_assets = response.get("assets", [])
                    logger.info(f"Direct lookup retrieved {len(all_assets)} assets to search through")
                except Exception as e:
                    logger.error(f"Error fetching assets for direct lookup: {e}")
            ...
            for asset_name in names_list:
                try:
                    ...
                    if direct_lookup and all_assets:
                        logger.info(f"Searching for {asset_name} in local asset list")
                        ...
                        if matching_assets:
                            asset = matching_assets[0]
                            logger.info(f"Direct lookup found asset {asset_name} with ID: {asset.get('id')}")
                        else:
                            ...
                            if approx_matches:
                                asset = approx_matches[0]
                                logger.info(f"Direct lookup found approximate match for {asset_name}: {asset.get('Name')} (ID: {asset.get('id')})")
                    else:
                        # Use JSON filter lookup method
                        logger.info(f"Using JSON filter lookup for asset: {asset_name}")
                        ...
                        try:
                            ...
                            if items and len(items) > 0:
                                ...
                                asset = items[0]
                                logger.info(f"Found exact match for {asset_name}")
                            else:
                                ...
                                if items and len(items) > 0:
                                    # Take the first match
                                    asset = items[0]
                                    logger.info(f"Found LIKE match for {asset_name}")
                                else:
                                    ...
                                    if '-' in asset_name:
                                        ...
                                        if items and len(items) > 0:
                                            ...
                                            if exact_matches:
                                                asset = exact_matches[0]
                                                logger.info(f"Found exact match within prefix results for {asset_name}")
                                            else:
                                                # Just take first one as approximate match
                                                asset = items[0]
                                                logger.info(f"Found approximate match for {asset_name}: {asset.get('Name')}")
                                        else:
                                            # Fall back to the original method if JSON approach fails
                                            logger.info("JSON filter method failed, falling back to find_asset_by_name")
                                            asset = find_asset_by_name(asset_name, settings)
                                    else:
                                        # Fall back to the original method if JSON approach fails
                                        logger.info("JSON filter method failed, falling back to find_asset_by_name")
                            ...
                            logger.warning(f"JSON filter lookup failed: {json_error}, falling back to find_asset_by_name")
                            # Fall back to the original method if JSON approach fails with an exception
                            asset = find_asset_by_name(asset_name, settings)
                    
                    if asset:
                        logger.info(f"Found asset {asset_name} with ID: {asset.get('id')}")
                        assets.append(asset)
                    else:
                        logger.error(f"Asset not found: {asset_name}")
                        failed_names.append(asset_name)
                except Exception as e:
                    logger.error(f"Error processing asset name {asset_name}: {e}")
                    failed_names.append(asset_name)
            ...
            # Log success
            logger.info(f"Successfully found {len(assets)} assets")
        ...
        # If we're using a query, search for assets
            ...
            if re.match(r'^[A-Za-z0-9]+$', query):  # Simple prefix like W12
                try:
                    logger.info("Query looks like a prefix, trying JSON filter approach")
                    ...
                    logger.info(f"Making POST request with filter: {json.dumps(filter_data)}")
                    response = requests.post(url, headers=headers, json=filter_data)
                    response.raise_for_status()
                    ...
                    if items and len(items) > 0:
                        logger.info(f"JSON filter found {len(items)} assets")
                        assets = items
                        ...
                        logger.info("JSON filter found no assets, falling back to standard search")
                        assets = search_assets(query, settings)
                except Exception as e:
                    logger.warning(f"JSON filter search failed: {e}, falling back to standard search")
            ...
            logger.info(f"Query found {len(assets)} assets")
        ...
        for asset in assets:
            ...
            rt_asset_url = f"https://tickets.wc-12.com/Asset/Display.html?id={asset_id}"
            logger.debug(f"QR code URL for asset {asset_id}: {rt_asset_url}")
        ...
        logger.info(f"Rendering batch_labels.html with {len(labels_data)} labels")
        ...
        try:
            response = render(request, 'labels/batch_labels.html', **context)
            logger.info(f"Successfully rendered batch_labels.html, response length: {len(response)}")
            return response
        except Exception as template_error:
            logger.error(f"Error rendering template: {template_error}")
            return render(request, 'batch_labels_form.html', 
                                  error=f"Error rendering labels: {str(template_error)}")
        
    except Exception as e:
        import traceback
        error_traceback = traceback.format_exc()
        logger.error(f"Error processing batch labels: {e}")
        logger.error(f"Traceback: {error_traceback}")
...
def custom_JsonHttpResponse(data):
    """
    Custom version of JsonResponse that handles types that normally can't be serialized.
    """
    import json
...
def test_api_methods(request):
    ...
    try:
        ...
        try:
            ...
            logger.info(f"Testing direct GET request to: {url}")
        ...
        try:
            ...
            logger.info(f"Testing direct POST request to: {url} with data: {data}")
        ...
        try:
            ...
            logger.info(f"Testing direct POST request to: {url} with data as string: {json.dumps(data)}")
        ...
        try:
            ...
            logger.info(f"Testing POST with form-urlencoded: {url} with data: {form_data}")
        ...
        try:
            assets = search_assets(query, settings, try_post_fallback=False)
        ...
        return custom_JsonHttpResponse(results)
        
    except Exception as e:
        import traceback
        return custom_JsonHttpResponse({
...
def debug_asset(request):
    ...
    if not asset_name:
        return JsonResponse({"error": "Please provide an asset name parameter"}, status=400)
    ...
    try:
        ...
        for method in search_methods:
            try:
                ...
                response = rt_api_request("GET", f"/assets?query={encoded_query}", settings)
        ...
        try:
            ...
            response = rt_api_request("GET", f"/assets?query={direct_query}", settings)
            all_assets = response.get("assets", [])
...
def search_assets_route(request):
    """
    ...
    limit = request.GET.get('limit', 20, type=int)
    ...
    try:
        ...
        logger.info(f"Searching for assets with term '{search_term}' using JSON filter format")
        ...
        logger.info(f"Equivalent curl command: {curl_cmd}")
        ...
        try:
            ...
            logger.info(f"POST request returned response: {json.dumps(result)}")
            
            # Try to determine what kind of response we got
            if 'items' in result:
                logger.info(f"Response has 'items' field with {len(result.get('items', []))} items")
            elif 'assets' in result:
                logger.info(f"Response has 'assets' field with {len(result.get('assets', []))} assets")
            else:
                logger.info("Response does not have 'items' or 'assets' field")
            ...
            logger.info(f"Found {len(items)} assets using JSON filter")
            
            # Log the first item to see its structure
            if items and len(items) > 0:
                logger.info(f"First item structure: {json.dumps(items[0])}")
            ...
            for item in items:
                try:
                    ...
                    asset_id = item.get("id")
                    if not asset_id:
                        logger.warning(f"Item has no ID, skipping: {item}")
                        continue
                        
                    # Fetch complete asset details
                    logger.debug(f"Fetching details for asset ID: {asset_id}")
                    asset_data = fetch_asset_data(asset_id, settings)
                    ...
                    if isinstance(catalog_info, dict):
                        ...
                        if catalog_id:
                            try:
                                catalog_data = rt_api_request("GET", f"/catalog/{catalog_id}", config=settings)
                                catalog_name = catalog_data.get("Name")
                                logger.debug(f"Found catalog name: {catalog_name} for ID: {catalog_id}")
                            except Exception as catalog_error:
                                logger.warning(f"Error fetching catalog details: {catalog_error}")
                    ...
                    logger.error(f"Error fetching details for asset {item.get('id')}: {detail_error}")
            ...
            logger.warning(f"JSON filter query failed: {json_error}, trying standard search")
        ...
        try:
            ...
            logger.warning(f"LIKE query failed, falling back to manual filtering: {e}")
            all_query = "id>0 LIMIT 1000"
            encoded_all_query = urllib.parse.quote(all_query)
            response = rt_api_request("GET", f"/assets?query={encoded_all_query}", settings)
            all_assets = response.get("assets", [])
        ...
        for asset in assets:
            ...
            try:
                ...
                logger.error(f"Error fetching details for asset {asset_id}: {detail_error}")
                # Include basic info even if details fetch fails
                results.append({
                    "id": asset_id,
...
def direct_lookup_route(request):
    """
    ...
    asset_name = request.GET.get('name')
    if not asset_name:
        return HttpResponse('{"error": "Missing name parameter"}', mimetype='application/json')
    matching_assets = []
    partial_matches = []
    try:
        ...
        logger.info(f"Searching for asset with name '{asset_name}' using GET query")
        
        # Make request using GET with query parameters
        query_url = f"{url}?query={urllib.parse.quote(query)}"
        logger.debug(f"Full URL: {query_url}")
...
def fetch_assets_direct(request):
    """
...
def get_asset_info(request):
    ...
    if not asset_id:
        return JsonResponse({"error": "Please provide an asset ID with the 'id' parameter"})
...
def list_assets(request):
    ...
    try:
        ...
        response = rt_api_request("GET", f"/assets?query={urllib.parse.quote(query)}", settings)
        assets = response.get("assets", [])
...
def search_assets_json(request):
    """
    ...
    try:
        ...
        filter_conditions = json.loads(request.body.decode("utf-8")) if request.body else None
        ...
        logger.info(f"Received asset filter conditions: {filter_conditions}")
        ...
        import requests
        logger.debug(f"Making POST request to RT API: {url}")
        ...
        # Process the response
        result = response.json()
        logger.debug(f"RT API returned {len(result.get('items', []))} items")
        ...
        return custom_JsonHttpResponse(sanitize_json(result))
        
    except Exception as e:
        import traceback
        logger.error(f"Error processing asset search: {e}")
        return custom_JsonHttpResponse({
...
def direct_print(request):
    ...
    asset_id = request.GET.get('id')
    if not asset_id:
        return JsonResponse({"error": "Please provide an asset ID with the 'id' parameter"}, status=400)
    ...
    try:
        ...
        # Generate QR Code with the RT URL
        rt_asset_url = f"{getattr(settings, 'RT_URL')}/Asset/Display.html?id={asset_id}"
        ...
        return render(request, "label.html", **asset_label_data)
        
    except Exception as e:
        logger.error(f"Error in direct print: {e}")
        return JsonResponse({
