...
from common.rt_api import get_assets_by_owner, fetch_asset_data, fetch_user_data, rt_api_request
import logging
import requests
import json
import time
...
def asset_checkin(request):
    """Display the device check-in form without a pre-filled asset name"""
    return render(request, 'device_checkin.html')

def asset_details(request, asset_name):
    """Display asset details and check-in form"""
    return render(request, 'device_checkin.html', asset_name=asset_name)

def get_asset_info(request, asset_name):
    ...
    try:
        ...
        base_url = getattr(settings, 'RT_URL', None)
        api_endpoint = getattr(settings, 'API_ENDPOINT', None)
        token = getattr(settings, 'RT_TOKEN', None)
        ...
        if set_owner_to_nobody:
            try:
                ...
                try:
                    ...
                    instance_logs_dir = os.path.join(settings.BASE_DIR, 'logs')
...
def checkin_logs(request):
    ...
    try:
        ...
        return render(request, 'devices/checkin_logs.html', logs=file_logs)
        ...
        try:
            return render(request, 'error.html', error=f"Failed to load check-in logs: {str(e)}")
        except Exception:
...
def download_checkin_log(filename):
    ...
    try:
        ...
        nested_logs_dir = os.path.join(settings.BASE_DIR, 'logs', 'logs')
...
def preview_checkin_log(filename):
    ...
    try:
        ...
        per_page = request.GET.get('per_page', 50, type=int)  # Show 50 rows per page by default
